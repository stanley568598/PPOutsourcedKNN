...
# First let's retrieve the decision path of each sample. 
# The decision_path method allows to retrieve the node indicator functions. 
...
# Now, it's possible to get the tests that were used to predict a sample or a group of samples. 

# First, let's make it for the sample.
...
for node_id in node_index:
    ...
    if leave_id[sample_id] == node_id:
        ...
        print("decision id node %s : (X[%s, %s] (= %s) %s %s)"
            % (node_id, 
                sample_id, feature[node_id], X_test[sample_id, feature[node_id]], 
